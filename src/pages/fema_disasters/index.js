...
class FemaDisasters extends React.Component {
    ...
    processData(){
        if(Object.keys(this.props.falcorCache).length > 0){
            ...
            Object.keys(graph).filter(d => d!=='$__path').forEach(item =>{
                data.push(
                    ...
                    if(graph[item][attribute]){
                        out[attribute] =  graph[item][attribute].value || 0
                    }
            ...
            data.forEach(d =>{
                d['ihp_details_total'] = d['total_amount_ihp_approved'] + d['total_amount_ona_approved']
                d['pa_details_total'] = d['total_obligated_amount_pa']
                d['hgmp_details_total'] = d['total_obligated_amount_hmgp']
